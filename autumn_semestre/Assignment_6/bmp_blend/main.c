...
BitMap *
CreateBMP (uint32_t width, uint32_t height)
{
  BitMap *bmp = NULL;
  ...
  bmp->data_
      = (uint8_t *)calloc (bmp->header_.img_data_size_, sizeof (uint8_t));
  if (bmp->data_ == NULL)
    {
      free (bmp);
      return NULL;
...
ReadImageFile (const char *filename)
{
  ...
  bmp->data_
      = (uint8_t *)malloc (bmp->header_.width_ * bmp->header_.height_ * 4);
  if (bmp->data_ == NULL)
    {
      fclose (f);
      free (bmp);
  ...
  if (fread (bmp->data_, sizeof (uint8_t),
             bmp->header_.width_ * bmp->header_.height_ * 4, f)
...
void
WriteToImageFile (BitMap *bmp, const char *filename)
{
...
uint32_t
height (const BitMap *bmp)
{
  return bmp->header_.height_;
}

uint32_t
width (const BitMap *bmp)
{
  return bmp->header_.width_;
}

__m128d
ChannelMaskToPackedDouble (__m128i input, __m128i mask)
{
...
__m128i
AssembleFourPixels (__m128d a, __m128d r, __m128d g, __m128d b)
{
  const __m128i alpha_store
      = _mm_setr_epi8 (0, 0xff, 0xff, 0xff, 1, 0xff, 0xff, 0xff, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  const __m128i red_store
      = _mm_setr_epi8 (0xff, 0, 0xff, 0xff, 0xff, 1, 0xff, 0xff, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  const __m128i green_store
      = _mm_setr_epi8 (0xff, 0xff, 0, 0xff, 0xff, 0xff, 1, 0xff, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  const __m128i blue_store
      = _mm_setr_epi8 (0xff, 0xff, 0xff, 0, 0xff, 0xff, 0xff, 1, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
...
__m128i
AlphaBlend (__m128i bg, __m128i fg)
{
  const __m128i get_alpha
      = _mm_setr_epi8 (0xff, 0xff, 0xff, 0, 0xff, 0xff, 0xff, 4, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  const __m128i get_red
      = _mm_setr_epi8 (0xff, 0xff, 0xff, 1, 0xff, 0xff, 0xff, 5, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  const __m128i get_green
      = _mm_setr_epi8 (0xff, 0xff, 0xff, 2, 0xff, 0xff, 0xff, 6, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  const __m128i get_blue
      = _mm_setr_epi8 (0xff, 0xff, 0xff, 3, 0xff, 0xff, 0xff, 7, 0xff, 0xff,
                       0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  ...
  __m128d res_red = _mm_add_pd (_mm_mul_pd (bg_red, inv_alpha),
                                _mm_mul_pd (fg_red, fg_alpha));
  ...
  __m128d res_blue = _mm_add_pd (_mm_mul_pd (bg_blue, inv_alpha),
                                 _mm_mul_pd (fg_blue, fg_alpha));
...
BitMap *
BlendImages (const BitMap *bg, const BitMap *fg, uint32_t x, uint32_t y)
{
      ...
      for (uint32_t j = 0; j < width (fg); j += 2)
        {
          ...
          __m128i under
              = _mm_loadu_si128 ((const __m128i *)(bg->data_ + bg_pos));
          __m128i over
              = _mm_loadu_si128 ((const __m128i *)(fg->data_ + fg_pos));
...
void
PrintPixel (const BitMap *bmp, uint32_t x, uint32_t y)
{
